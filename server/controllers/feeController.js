...
// @desc    Get vehicle fee balance for a student
// @route   GET /api/fees/vehicle-balance/:id
// @access  Private (Admin/Staff)
const getVehicleFeeBalance = async (req, res) => {
    try {
        const studentId = req.params.id;
        const student = await Student.findById(studentId);
        if (!student) {
        ...
        const vehicleFees = await Fee.find({ 
            student: studentId, 
            feeType: { $regex: /^Vehicle Fee/i } 
        });

        const totalPaid = vehicleFees.reduce((sum, fee) => sum + fee.amount, 0);
        const totalExpected = (student.paidVehicleMonths?.length || 0) * (student.monthlyConveyanceFee || 0);
