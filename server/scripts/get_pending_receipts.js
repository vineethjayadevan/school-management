...
async function getPendingReceipts() {
    try {
            ...
            if (particulars && typeof particulars === 'string') {
                ...
                if (admNo) {
                    ...
                    if (amount === 6500) {
                        ...
                        // Rule 3: Any amount >= 2000 is processed (Tuition fees)
                        isProcessed = true;
                    } else {
        ...
        const outputPath = 'C:\\Users\\vinee\\Desktop\\AG Projects\\school-management\\final_pending_receipts.txt';
        fs.writeFileSync(outputPath, pendingRows.join('\n'));
        console.log(`Found ${pendingRows.length} final pending entries. Saved to txt.`);
