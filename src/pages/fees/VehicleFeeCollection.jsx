...
export default function VehicleFeeCollection() {
    ...
    const [vehicleBalance, setVehicleBalance] = useState(null);
    ...
    const handleSelectStudent = async (student) => {
        ...
        setStudents([]);
        setSearchQuery('');
        setVehicleBalance(null);
        try {
            const res = await api.get(`/fees/vehicle-balance/${student._id}`);
            setVehicleBalance(res.data);
        } catch (error) {
    ...
    useEffect(() => {
        if (selectedStudent) {
            const base = selectedStudent.monthlyConveyanceFee * selectedMonths.length;
            const bal = vehicleBalance ? vehicleBalance.balance : 0;
            const net = Math.max(0, base - bal);
            setManualAmount(net.toString());
        }
    }, [selectedMonths, selectedStudent, vehicleBalance]);
    ...
    const handlePreviewFee = () => {
        ...
        const baseAmount = selectedStudent.monthlyConveyanceFee * selectedMonths.length;
        const finalAmount = Number(manualAmount);

        let breakdown = selectedMonths.map(month => ({
            feeType: `Vehicle Fee - ${month}`,
            amount: selectedStudent.monthlyConveyanceFee
        }));

        if (vehicleBalance && vehicleBalance.balance < 0) {
            breakdown.push({ feeType: 'Previous Arrears', amount: Math.abs(vehicleBalance.balance) });
        } else if (vehicleBalance && vehicleBalance.balance > 0) {
            breakdown.push({ feeType: 'Previous Advance (Deducted)', amount: -Math.abs(vehicleBalance.balance) });
        }
    ...
    const handleFinalPayment = async () => {
        try {
            ...
            // Refresh balance
            try {
                const balRes = await api.get(`/fees/vehicle-balance/${selectedStudent._id}`);
                setVehicleBalance(balRes.data);
            } catch(e) {}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                            ...
                            {vehicleBalance && (vehicleBalance.isArrears || vehicleBalance.isSurplus) && (
                                <div className={`p-4 rounded-xl mb-6 border ${vehicleBalance.isArrears ? 'bg-red-50 border-red-100' : 'bg-green-50 border-green-100'}`}>
                                    <h4 className={`text-sm font-bold flex justify-between ${vehicleBalance.isArrears ? 'text-red-800' : 'text-green-800'}`}>
                                        <span>{vehicleBalance.isArrears ? 'Previous Arrears (Pending)' : 'Previous Advance (Surplus)'}</span>
                                        <span>₹{Math.abs(vehicleBalance.balance).toLocaleString()}</span>
                                    </h4>
                                    <p className={`text-xs mt-1 ${vehicleBalance.isArrears ? 'text-red-600' : 'text-green-600'}`}>
                                        {vehicleBalance.isArrears 
                                            ? 'This amount will be added to the next payment.' 
                                            : 'This amount will be automatically deducted from the next payment.'}
                                    </p>
                                </div>
                            )}
                            ...
                            <div className="space-y-4 mb-6">
                                ...
                                <div className="pt-4 border-t border-slate-100">
                                    <div className="flex justify-between items-end mb-4">
                                        <span className="text-slate-800 font-bold">Base Amount</span>
                                        <span className="text-xl font-bold text-slate-600">₹{totalAmount.toLocaleString()}</span>
                                    </div>
                                    <div className="mb-4">
                                        <label className="block text-xs font-semibold text-slate-700 mb-2 uppercase tracking-wider">Net Amount to Collect (Editable)</label>
                                        <input
                                            type="number"
                                            value={manualAmount}
                                            onChange={(e) => setManualAmount(e.target.value)}
                                            className="w-full p-3 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-lg font-bold text-blue-600"
                                            min="0"
                                        />
                                        {vehicleBalance && vehicleBalance.balance !== 0 && (
                                            <p className="text-xs text-slate-500 mt-1">
                                                * Includes previous {vehicleBalance.balance < 0 ? 'arrears' : 'advance'}
                                            </p>
                                        )}
                                    </div>
                                </div>
